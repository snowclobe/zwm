...
/* Computes up to `n` window rectangles for a bottom-stack tiling layout
 * (master row on top, stack row below) and writes them into `out` (capacity
 * `out_cap` entries). Returns the number of rectangles actually written
 * (min(n, out_cap)). Pure function, no side effects, safe to call with
 * out == NULL only when out_cap == 0. */
uint32_t zov_layout_bstack(uint32_t n,
